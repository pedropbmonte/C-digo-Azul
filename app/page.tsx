...
export default function CodigoAzulGame() {
  ...
  // --- GERAÇÃO DE CASO COM STRING PURIFICADA ---
  const fetchScenarioFromAI = async (stageNum: number) => {
    ...
    const promptText = "Você é o reitor sênior da escola de negócios 'Código Azul'. Gere um Estudo de Caso Prático inédito para a fase " + (stageNum + 1) + " de 10, com profundidade teórica extrema, citando CPCs, IFRS ou normas tributárias aplicáveis ao nível " + currentLevel.title + ". A empresa se chama " + companyName + " com Caixa R$ " + caixa + ", Margem " + margem + "% e Compliance " + compliance + "%. Retorne APENAS um JSON estrito, sem formatação markdown, contendo: sector, criticality, title, theory (longo e normativo), context (problema real), character (autoridade), e options (array com 3 objetos contendo id, text, xp, isBest, impacts {caixa, margem, compliance}, feedback).";
    ...
    try {
      ...
      setCurrentScenario({
        ...
        criticality: "Extrema",
        title: "Gestão Estratégica de Capital de Giro",
        ...
        context: "Sua empresa apresentou desalinhamento nas contas a pagar e receber, exigindo intervenção direta do CFO.",
