...
export default function CodigoAzulGame() {
  ...
  const [isProcessing, setIsProcessing] = useState(false); // Trava anti-glitch de duplo clique

  const [saveStatus, setSaveStatus] = useState<string | null>(null);
  const [isLoading, setIsLoading] = useState(true);

  // MUDANÇA DE CHAVE PARA V4 (FORÇA O RESET DE PONTOS INFLADOS ANTIGOS)
  const saveToDB = () => {
    if (!nickname) return;
    const db = JSON.parse(localStorage.getItem('codigoAzul_Corp_v4') || '{}');
    if(db[nickname]) {
      db[nickname].data = { playerName, companyName, xp, currentStage, sessionScenarios };
      localStorage.setItem('codigoAzul_Corp_v4', JSON.stringify(db));
    }
  };
  ...
  const handleResetCareer = () => {
    if(confirm("Operação irreversível. Liquidar empresa e resetar patentes para zero?")) {
      const newCompany = generateCompanyName();
      const randomizedJourney = shuffleArray(allScenarios).slice(0, 12);
  ...
  useEffect(() => {
    if (timeLeft === 0 && !feedback && !promotionPending && gameStarted && currentLevel.hasTimer) {
      handleChoice(-15, "TEMPO ESGOTADO. O mercado puniu sua hesitação corporativa severamente.", true);
    }
  // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  const handleChoice = (baseXpGained: number, feedbackText: string, isTimeout: boolean = false) => {
    if (isProcessing) return; // Trava contra cliques duplos que geram "XP fantasma"
    setIsProcessing(true);

    let bonus = 0;
    // Bônus de tempo drasticamente reduzido (max 4 pts)
    if (baseXpGained > 0 && !isTimeout && currentLevel.hasTimer) {
      if (timeLeft >= 45) bonus = 4; else if (timeLeft >= 30) bonus = 2; 
    }
  ...
  const handleNextStageOrPromotion = () => {
    if (promotionPending) { 
      setFeedback(null); 
      playPromotionSound(); 
      setIsProcessing(false); // Libera a trava visual após clicar no botão de prosseguir
      return; 
    }
  ...
  const proceedToNextQuestion = () => {
    ...
    setIsProcessing(false); // Libera para a próxima rodada
    
    if (currentStage < sessionScenarios.length - 1) {
      setCurrentStage(prev => prev + 1);
    } else {
      alert(`Sessão de Auditoria Concluída!\n\nSua Patente Consolidada: ${currentLevel.title}\nXP Total Acumulado: ${xp} XP.\n\nO sistema sorteará novos cenários para a continuação da sua maratona executiva.`);
            ...
            <div className="space-y-4">
              ...
              {scenario.options.map((option: any, index: number) => (
                <button
                  ...
                  className={`w-full text-left p-6 rounded-xl bg-[#020617]/50 border border-slate-700/50 transition-all group relative overflow-hidden ${isProcessing ? 'opacity-50 cursor-not-allowed' : 'hover:border-cyan-500/50 hover:bg-[#081229] hover:shadow-[0_0_20px_rgba(6,182,212,0.1)]'}`}
                >
